...
export default function SettingsPage() {
  ...
  const saveHousehold = useMutation({
    ...
    onSuccess: (data) => queryClient.setQueryData(['household'], data),
  })

  const addCategory = useMutation({
    mutationFn: createShoppingCategory,
    onSuccess: async () => {
      setCategoryName('')
      await queryClient.invalidateQueries({ queryKey: ['shopping-categories'] })
    },
  })
  ...
  function submitHousehold(event: FormEvent) {
    event.preventDefault()
    saveHousehold.mutate({ name: householdName, default_servings: defaultServings })
  }

  function submitCategory(event: FormEvent) {
    event.preventDefault()
    addCategory.mutate({ name: categoryName, sort_order: (categories.data?.length ?? 0) * 10 + 10 })
  }
  ...
  function submitLocation(event: FormEvent) {
    ...
    addLocation.mutate({
      ...
      parent_location_id: parentLocationId ? Number(parentLocationId) : null,
      sort_order: (locations.data?.length ?? 0) * 10 + 10,
    })
  }
  ...
  return (
    <section className="settings-page">
      <div className="page-card">
        ...
        <h1>Household & Reference Data</h1>
        <p>Configure the shared defaults used by recipes, inventory, planning, and shopping.</p>
      </div>
      ...
      <div className="settings-grid">
        <section className="settings-card">
          ...
          <form onSubmit={submitHousehold} className="form-stack">
            ...
            <button type="submit" disabled={saveHousehold.isPending}>Save household</button>
          </form>
        </section>
        ...
        <section className="settings-card settings-card-wide">
          ...
          <form onSubmit={submitLocation} className="location-form">
            <input
            ...
            <select value={locationType} onChange={(event) => setLocationType(event.target.value)}>
              {LOCATION_TYPES.map((type) => <option key={type}>{type}</option>)}
            </select>
