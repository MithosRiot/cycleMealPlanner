...
export type Equipment = { id: number; household_id: number; name: string; category: string; notes: string | null; active: boolean }
...
export type RecipeAdvancePrepInput = { title: string; lead_time_minutes: number; duration_minutes: number | null; instructions: string | null; prep_group_key: string | null; sort_order: number }
export type RecipeAdvancePrep = { id: number; recipe_id: number; prep_group_id: number | null; title: string; lead_time_minutes: number; duration_minutes: number | null; instructions: string | null; sort_order: number }
export type RecipeEquipmentInput = { equipment_id: number; quantity: number; notes: string | null; sort_order: number }
export type RecipeEquipment = RecipeEquipmentInput & { id: number; recipe_id: number }
export type RecipeIngredientSubstitutionInput = { substitute_ingredient_id: number; ratio: string; preferred: boolean; notes: string | null; sort_order: number }
export type RecipeIngredientSubstitution = RecipeIngredientSubstitutionInput & { id: number; recipe_ingredient_id: number }
export type RecipeIngredientInput = { ingredient_id: number; prep_group_key: string | null; quantity: string; unit_id: number; display_text: string | null; preparation: string | null; prep_method: string | null; prep_size: string | null; prep_state: string | null; optional: boolean; scaling_mode: 'LINEAR' | 'FIXED' | 'ROUND_UP' | 'MANUAL'; required_state: string; sort_order: number; notes: string | null; substitutions: RecipeIngredientSubstitutionInput[] }
export type RecipeIngredient = Omit<RecipeIngredientInput, 'prep_group_key' | 'substitutions'> & { id: number; recipe_id: number; prep_group_id: number | null; substitutions: RecipeIngredientSubstitution[] }
export type RecipeVariantOverrideInput = { recipe_ingredient_id: number; quantity: string | null; unit_id: number | null; substitution_id: number | null; preparation: string | null; prep_method: string | null; prep_size: string | null; prep_state: string | null; notes: string | null }
export type RecipeVariantOverride = RecipeVariantOverrideInput & { id: number; variant_id: number }
export type RecipeVariantInput = { name: string; notes: string | null; active: boolean; sort_order: number; overrides: RecipeVariantOverrideInput[] }
export type RecipeVariant = RecipeVariantInput & { id: number; recipe_id: number; overrides: RecipeVariantOverride[] }
export type RecipeInput = { name: string; description: string | null; base_servings: string; serving_unit: string; yield_quantity: string | null; yield_unit_id: number | null; prep_time_minutes: number | null; cook_time_minutes: number | null; notes: string | null; favorite: boolean; meal_types: string[]; tag_ids: number[]; prep_groups: RecipePrepGroupInput[]; advance_prep: RecipeAdvancePrepInput[]; equipment: RecipeEquipmentInput[]; ingredients: RecipeIngredientInput[]; active?: boolean }
export type Recipe = { id: number; household_id: number; name: string; description: string | null; base_servings: string; serving_unit: string; yield_quantity: string | null; yield_unit_id: number | null; prep_time_minutes: number | null; cook_time_minutes: number | null; notes: string | null; favorite: boolean; active: boolean; meal_types: string[]; tags: Tag[]; prep_groups: RecipePrepGroup[]; advance_prep: RecipeAdvancePrep[]; equipment: RecipeEquipment[]; ingredients: RecipeIngredient[] }
export type ScaledRecipeIngredient = { recipe_ingredient_id: number; ingredient_id: number; canonical_ingredient_id: number; substitution_id: number | null; prep_group_id: number | null; quantity: string; unit_id: number; unit_code: string; scaling_mode: string; manual_review: boolean; preparation: string | null; prep_method: string | null; prep_size: string | null; prep_state: string | null }
export type RecipeScaleResponse = { recipe_id: number; base_servings: string; requested_servings: string; scale_factor: string; variant_id: number | null; ingredients: ScaledRecipeIngredient[] }
export type InventoryLot = { id: number; household_id: number; ingredient_id: number | null; source_type: 'INGREDIENT' | 'LEFTOVER' | 'RECIPE_OUTPUT'; source_id: number | null; source_name: string | null; location_id: number; quantity: string; unit_id: number; purchase_date: string | null; opened_date: string | null; expiration_date: string | null; frozen_date: string | null; thawed_date: string | null; notes: string | null }
...
export const fetchEquipment = (includeInactive = false): Promise<Equipment[]> => jsonRequest(`/api/equipment${includeInactive ? '?include_inactive=true' : ''}`)
export const createEquipment = (input: { name: string; category: string; notes: string | null }): Promise<Equipment> => jsonRequest('/api/equipment', { method: 'POST', body: JSON.stringify(input) })
export const updateEquipment = (item: Equipment): Promise<Equipment> => jsonRequest(`/api/equipment/${item.id}`, { method: 'PUT', body: JSON.stringify({ name: item.name, category: item.category, notes: item.notes, active: item.active }) })
export const archiveEquipment = (id: number): Promise<void> => jsonRequest(`/api/equipment/${id}`, { method: 'DELETE' })
...
export const fetchRecipeVariants = (recipeId: number, includeInactive = false): Promise<RecipeVariant[]> => jsonRequest(`/api/recipes/${recipeId}/variants${includeInactive ? '?include_inactive=true' : ''}`)
export const createRecipeVariant = (recipeId: number, input: RecipeVariantInput): Promise<RecipeVariant> => jsonRequest(`/api/recipes/${recipeId}/variants`, { method: 'POST', body: JSON.stringify(input) })
export const updateRecipeVariant = (recipeId: number, variantId: number, input: RecipeVariantInput): Promise<RecipeVariant> => jsonRequest(`/api/recipes/${recipeId}/variants/${variantId}`, { method: 'PUT', body: JSON.stringify(input) })
export const archiveRecipeVariant = (recipeId: number, variantId: number): Promise<void> => jsonRequest(`/api/recipes/${recipeId}/variants/${variantId}`, { method: 'DELETE' })
export const scaleRecipe = (id: number, requestedServings: string, unitOverrides: Record<number, string> = {}, substitutionOverrides: Record<number, number> = {}, variantId: number | null = null): Promise<RecipeScaleResponse> => jsonRequest(`/api/recipes/${id}/scale`, { method: 'POST', body: JSON.stringify({ requested_servings: requestedServings, unit_overrides: unitOverrides, substitution_overrides: substitutionOverrides, variant_id: variantId }) })
