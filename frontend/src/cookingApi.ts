...
export type CookingStepInput = { title: string; instructions: string | null; prep_group_id: number | null; sort_order: number; timers: CookingTimerInput[] }
export type CookingStep = Omit<CookingStepInput, 'timers'> & { id: number; recipe_id: number; prep_group_name: string | null; timers: CookingTimer[] }
...
export type CookingTimerRuntime = { timer_id: number; label: string; duration_seconds: number; notes: string | null; sort_order: number; status: 'READY' | 'RUNNING' | 'PAUSED' | 'COMPLETED'; remaining_seconds: number; ends_at_epoch: number | null }
export type CookingModeStep = { step_id: number; component_index: number; meal_recipe_id: number; recipe_id: number; recipe_name: string; title: string; instructions: string | null; prep_group_id: number | null; prep_group_name: string | null; step_number: number; total_steps: number; ingredients: CookingIngredient[]; timers: CookingTimerRuntime[] }
