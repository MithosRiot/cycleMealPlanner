...
export default function InventoryPage() {
  ...
  const split = useMutation({ mutationFn: ({ lot, value, target, note }: { lot: InventoryLot; value: string; target: number; note: string }) => splitInventory(lot.id, value, target, note), onSuccess: refresh })
  ...
  const lotName = (lot: InventoryLot) => lot.source_type === 'INGREDIENT'
    ? ingredientNames.get(lot.ingredient_id as number) ?? `Ingredient ${lot.ingredient_id}`
    : lot.source_name ?? (lot.source_type === 'LEFTOVER' ? `Leftover ${lot.source_id}` : `Recipe output ${lot.source_id}`)

  function submitLot(event: FormEvent) {
    event.preventDefault()
    createLot.mutate({ ingredient_id: Number(ingredientId), location_id: Number(locationId), quantity, unit_id: Number(unitId), purchase_date: purchaseDate || null, opened_date: null, expiration_date: expirationDate || null, frozen_date: null, thawed_date: null, notes: notes || null, transaction_type: purchase ? 'PURCHASE' : 'MANUAL_ADD' })
  }

  const grouped = useMemo(() => {
    const groups = new Map<string, InventoryLot[]>()
    for (const lot of lots.data ?? []) {
      const key = viewMode === 'ingredient' ? lotName(lot) : locationNames.get(lot.location_id) ?? `Location ${lot.location_id}`
  ...
  return <section className="inventory-page">
    ...
    <section className="settings-card">
      <div className="section-heading"><div><h2>Add inventory</h2><p className="muted-line">Record an Ingredient purchase or manual starting quantity. Leftovers and Recipe outputs are created by Meal completion.</p></div></div>
      <form className="inventory-add-form" onSubmit={submitLot}>
        <select value={ingredientId} onChange={(e) => setIngredientId(e.target.value)} required><option value="">Ingredient</option>{ingredients.data?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
        <select value={locationId} onChange={(e) => setLocationId(e.target.value)} required><option value="">Location</option>{locations.data?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
        ...
        <select value={unitId} onChange={(e) => setUnitId(e.target.value)} required><option value="">Unit</option>{units.data?.filter((item) => item.unit_family !== 'SERVING').map((item) => <option key={item.id} value={item.id}>{item.code}</option>)}</select>
    ...
    <section className="settings-card">
      <div className="inventory-toolbar">
        <div className="segmented-control"><button type="button" className={viewMode === 'ingredient' ? '' : 'button-secondary'} onClick={() => setViewMode('ingredient')}>By item</button><button type="button" className={viewMode === 'location' ? '' : 'button-secondary'} onClick={() => setViewMode('location')}>By location</button></div>
        <select value={ingredientFilter} onChange={(e) => setIngredientFilter(e.target.value)}><option value="">All items</option>{ingredients.data?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
...
function InventoryLotRow({ lot, itemName, locationName, unitCode, unitCodes, locationNames, locations, onAdjust, onDiscard, onMove, onSplit }: {
  ...
  return <div className="inventory-lot-row">
    <div className="inventory-lot-summary"><strong>{itemName} · Lot {lot.id}</strong><div className="muted-line">{lot.quantity} {unitCode} · {locationName}{lot.expiration_date ? ` · expires ${lot.expiration_date}` : ''}</div><div className="muted-line">Frozen date: {lot.frozen_date ?? '—'} · Thawed date: {lot.thawed_date ?? '—'}</div>{lot.source_type !== 'INGREDIENT' && <div className="muted-line">Produced stock · {lot.source_type === 'LEFTOVER' ? 'Leftover Meal servings' : 'Recipe output'}</div>}{lot.notes && <div className="muted-line">{lot.notes}</div>}</div>
