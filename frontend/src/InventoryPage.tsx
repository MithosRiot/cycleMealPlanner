  ...
  return (
    <section className="inventory-page">
      ...
      <section className="settings-card">
        <div className="section-heading"><div><h2>Add inventory</h2><p className="muted-line">Record an actual purchase or manual starting quantity.</p></div></div>
        <form className="inventory-add-form" onSubmit={submitLot}>
          ...
          <select value={unitId} onChange={(e) => setUnitId(e.target.value)} required><option value="">Unit</option>{units.data?.map((item) => <option key={item.id} value={item.id}>{item.code}</option>)}</select>
      ...
      <section className="settings-card">
        <div className="inventory-toolbar">
          <div className="segmented-control"><button type="button" className={viewMode === 'ingredient' ? '' : 'button-secondary'} onClick={() => setViewMode('ingredient')}>By ingredient</button><button type="button" className={viewMode === 'location' ? '' : 'button-secondary'} onClick={() => setViewMode('location')}>By location</button></div>
          <select value={ingredientFilter} onChange={(e) => setIngredientFilter(e.target.value)}><option value="">All ingredients</option>{ingredients.data?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
...
function InventoryLotRow({ lot, ingredientName, locationName, unitCode, unitCodes, locationNames, locations, onAdjust, onMove, onSplit }: {
  ...
  return (
    <div className="inventory-lot-row">
      ...
      <div className="inventory-actions">
        <select value={target} onChange={(e) => setTarget(e.target.value)}>{locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
      ...
      <div className="inventory-split-actions">
        ...
        <select value={splitTarget} onChange={(e) => setSplitTarget(e.target.value)}>{locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
      ...
      {showHistory && <div className="inventory-history">
