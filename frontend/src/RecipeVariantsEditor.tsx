...
export default function RecipeVariantsEditor({ recipe, ingredients, units }: { recipe: Recipe; ingredients: Ingredient[]; units: MeasurementUnit[] }) {
  ...
  return <section className="editor-card">
    ...
    <div className="recipe-ingredient-list">{variants.data?.map((variant) => <div className="recipe-ingredient-editor" key={variant.id}>
      ...
      {variant.overrides.map((override, overrideIndex) => { const row = recipe.ingredients.find((item) => item.id === override.recipe_ingredient_id); const sub = row?.substitutions.find((item) => item.id === override.substitution_id); return <div className="muted-line" key={`${variant.id}-${override.recipe_ingredient_id}-${overrideIndex}`}>{ingredientNames.get(row?.ingredient_id ?? 0) ?? 'Ingredient'}{override.quantity != null ? ` · qty ${override.quantity}` : ''}{override.unit_id != null ? ` · ${units.find((unit) => unit.id === override.unit_id)?.code ?? 'unit'}` : ''}{sub ? ` · substitute ${ingredientNames.get(sub.substitute_ingredient_id) ?? 'ingredient'}` : ''}</div> })}
    </div>)}</div>
