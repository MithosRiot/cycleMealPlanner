...
def _save_recipe(db: Session, recipe: Recipe, payload: RecipeCreate | RecipeUpdate) -> Recipe:
    ...
    for item in sorted(payload.advance_prep, key=lambda value: value.sort_order):
        ...
        if reminder_enabled and reminder_offset is None:
            reminder_offset = 15
        recipe.advance_prep.append(RecipeAdvancePrep(
    ...
    for item in sorted(payload.equipment, key=lambda value: value.sort_order):
        recipe.equipment.append(RecipeEquipment(
    ...
    for item in sorted(payload.ingredients, key=lambda value: value.sort_order):
        ...
        db.flush()
        new_by_canonical.setdefault(item.ingredient_id, model)
        for sub in sorted(item.substitutions, key=lambda value: value.sort_order):
            model.substitutions.append(RecipeIngredientSubstitution(
    ...
    for variant, rows in preserved_variants:
        for row in rows:
            ...
            variant.overrides.append(RecipeVariantIngredientOverride(
                recipe_ingredient_id=new_item.id,
...
def scale_recipe(recipe_id: int, payload: RecipeScaleRequest, db: Session = Depends(get_db)) -> dict:
    ...
    if payload.variant_id is not None:
        ...
        if variant is None:
            raise HTTPException(status_code=400, detail="Recipe variant not found or inactive")
    ...
    for item in recipe.ingredients:
        ...
        if source_unit is None:
            raise HTTPException(status_code=409, detail=f"Stored unit {source_unit_id} no longer exists")
