...
def _save_recipe(db: Session, recipe: Recipe, payload: RecipeCreate | RecipeUpdate) -> Recipe:
    ...
    db.add(recipe)
    for variant, _rows in preserved_variants:
        variant.overrides.clear()
    recipe.ingredients.clear(); recipe.advance_prep.clear(); recipe.equipment.clear(); recipe.prep_groups.clear(); recipe.meal_types.clear(); recipe.tags = []
    db.flush()
    ...
    for item in sorted(payload.ingredients, key=lambda value: value.sort_order):
        ...
        for sub in sorted(item.substitutions, key=lambda value: value.sort_order):
            model.substitutions.append(RecipeIngredientSubstitution(substitute_ingredient_id=sub.substitute_ingredient_id, ratio=sub.ratio, preferred=sub.preferred, notes=sub.notes.strip() if sub.notes else None, sort_order=sub.sort_order))
        db.flush()
    ...
    for variant, rows in preserved_variants:
        for row in rows:
            ...
            if row["substitute_ingredient_id"] is not None:
                match = next((sub for sub in new_item.substitutions if sub.substitute_ingredient_id == row["substitute_ingredient_id"]), None)
    ...
    try: db.commit()
        ...
        db.rollback(); raise HTTPException(status_code=409, detail="Recipe could not be saved") from exc
...
def scale_recipe(recipe_id: int, payload: RecipeScaleRequest, db: Session = Depends(get_db)) -> dict:
    ...
    variant_overrides = {row.recipe_ingredient_id: row for row in variant.overrides} if variant else {}
    scale_factor = Decimal(payload.requested_servings) / Decimal(recipe.base_servings)
    scaled_items: list[dict] = []

    for item in recipe.ingredients:
        variant_override = variant_overrides.get(item.id)
        source_quantity = Decimal(variant_override.quantity) if variant_override and variant_override.quantity is not None else Decimal(item.quantity)
        source_unit_id = variant_override.unit_id if variant_override and variant_override.unit_id is not None else item.unit_id
        source_unit = db.get(MeasurementUnit, source_unit_id)
        if source_unit is None: raise HTTPException(status_code=409, detail=f"Stored unit {source_unit_id} no longer exists")
        scaled_quantity, manual_review = scale_quantity(source_quantity, scale_factor, item.scaling_mode)
        ...
        elif variant_override and variant_override.substitution_id is not None:
            selected_substitution = next((sub for sub in item.substitutions if sub.id == variant_override.substitution_id), None)
        else:
        ...
        if selected_substitution is not None:
            scaled_quantity *= Decimal(selected_substitution.ratio); output_ingredient_id = selected_substitution.substitute_ingredient_id; substitution_id = selected_substitution.id
        ...
        scaled_items.append({
            "recipe_ingredient_id": item.id, "ingredient_id": output_ingredient_id, "canonical_ingredient_id": item.ingredient_id,
            "substitution_id": substitution_id, "prep_group_id": item.prep_group_id, "quantity": scaled_quantity,
            "unit_id": target_unit.id, "unit_code": target_unit.code, "scaling_mode": item.scaling_mode, "manual_review": manual_review,
            "preparation": variant_override.preparation if variant_override and variant_override.preparation is not None else item.preparation,
            "prep_method": variant_override.prep_method if variant_override and variant_override.prep_method is not None else item.prep_method,
            "prep_size": variant_override.prep_size if variant_override and variant_override.prep_size is not None else item.prep_size,
            "prep_state": variant_override.prep_state if variant_override and variant_override.prep_state is not None else item.prep_state,
        })
